...
#include "Flashlib.h"
uint8_t rxSym[1];
volatile bool Calibrated = false;  //  to calibrate phase
volatile bool Charge =false;

volatile bool CalibLB = false;
volatile bool WriteSN = false;
  ...
  //  HAL_UART_Receive_IT(huart, &charRx, 1);
    ...
    switch(rxSym[0])
        ...
        case 'c': //charge
        	...
        	break;
        case 's': //send Serial Number to load balancer
        	if(!WriteSN)
        	{
        		WriteSN = true;
        	...
        	break;
        case 'u': //calibrate Load Balancer
        	if(!CalibLB)
        	{
        		CalibLB =true;
        	}
        ...
        case 'e': // to erase previous saved SN in charger side
        	if(Flash_Erase_Page(0x0803F000)==HAL_OK)
        	{
        		char msge[]="E\n";
        		HAL_UART_Transmit(&huart2,(uint8_t*)msge,strlen(msge),HAL_MAX_DELAY);
        	}
        ...
        HAL_UART_Receive_IT(&huart2, rxSym, 1);
  }
